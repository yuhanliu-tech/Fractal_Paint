...
import { OceanFloorChunk } from "./oceanfloorchunk";
...
export class OceanFloor {
    ...
    constructor() {
        this.computeBindGroupLayout = renderer.device.createBindGroupLayout({
            label: "ocean floor compute layout",
            entries: [
                {   // world position
                    binding: 0,
        ...
        this.renderBindGroupLayout = renderer.device.createBindGroupLayout({
            label: "ocean floor render layout",
            entries: [
                { // displacement
                    binding: 0,
        ...
        this.computePipeline = renderer.device.createComputePipeline({
            label: "ocean floor compute pipeline",
            layout: renderer.device.createPipelineLayout({
                label: "ocean floor compute pipeline layout",
                bindGroupLayouts: [this.computeBindGroupLayout]
            }),
            compute: {
                module: renderer.device.createShaderModule({
                    label: "ocean floor compute shader",
                    code: shaders.oceanFloorComputeSrc
                }),
                entryPoint: "main"
            }
        });
    ...
    public computeTextures(encoder: GPUCommandEncoder, chunk: OceanFloorChunk) {
